...
void *parse_json_and_insert(char *filename, void **estrutura_dados, InserirCallback callback_inserir)
{
    ...
    memset(&temp_registro, 0, sizeof(Registro));
    ...
    for (i = 0; i < count; i++)
    {
        ...
        {
            strcpy(temp_registro.renamaut, "N/A");
        }
        ...
        printf("DEBUG: Inserindo registro - renamaut: %s, fab: %s, mod: %s\n", 
               temp_registro.renamaut, temp_registro.fab, temp_registro.mod);
